...
- `get_maker_orders_info()` – orders.rs:1123–1139
- `determine_perp_fulfillment_methods()` – orders.rs:1736–1752
...
Drift Protocol combines elements of traditional order books with innovative auction logic and a protocol-native AMM. The JIT auction mechanism facilitates efficient price discovery and real-time liquidity, while the vAMM ensures baseline market functionality. Together, these components create a decentralized and flexible execution model that balances taker execution quality with maker incentives and system robustness.
