...
These parameters define a predictable linear price curve over the auction duration
...
The vAMM serves as a backstop, activating when no DLOB or JIT orders are available/sufficient. It quotes prices centered around the oracle price and sticks to protocol-defined parameters. 
...
In some cases, the vAMM also participates in JIT fills to rebalance risk exposure, particularly when external makers decline to fill undesirable flow. This ensures continued solvency and smooth markets.
