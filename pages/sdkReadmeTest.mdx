...
⚠️ This is a workaround for `@drift-labs/sdk`. The SDK includes imports Node.js modules like fs, which aren't available in browsers.

### Fix

Trying to import the SDK in a frontend app will lead to errors like:
``` Module not found: Can't resolve 'fs' ```

#### Next.js 15 Turbo
If you're using Next.js 15 Turbo, modify your next.config.js:
...
Create a file at node-browser-compatibility/index.js with the following content:
```js
module.exports = {};
```

#### Older versions
If you’re using an older version of Next.js, modify your next.config.js to ignore those modules:
  ...
  webpack(config, { isServer }) {
    if (!isServer) {
      config.resolve.fallback = {
        ...
        dns: false, // Disables the polyfill for 'dns' module
      };
    }
...
And modify your package.json like this:
